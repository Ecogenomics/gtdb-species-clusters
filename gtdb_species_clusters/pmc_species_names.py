...
class PMC_SpeciesNames(object):
    ...
    def key_taxon(self, gid, taxonomy):
        ...
        genus = taxonomy[gid][TaxonRank.GENUS_INDEX]
        species = taxonomy[gid][TaxonRank.SPECIES_INDEX]
        ...
        if rid in unambiguous_sp_rids:
            ...
            if final_species in cur_gtdb_sp:
                ...
                if (cur_genomes[prev_sp_rid].is_effective_type_strain()
                    ...
                    self.log.error('Manual curation require to resolve name of genome {} as proposed name {} was assigned previously.'.format(
                        rid, final_species))
            ...
            if prev_gtdb_specific and is_placeholder_sp_epithet(prev_gtdb_specific):
                ...
                if rid in ncbi_misclassified_gids and ncbi_species in ncbi_species_type_strain_rid:
                    gtdb_family = final_taxonomy[rid][TaxonRank.FAMILY_INDEX]
                    type_rid = ncbi_species_type_strain_rid[ncbi_species]
                    type_strain_gtdb_family = final_taxonomy[type_rid][TaxonRank.FAMILY_INDEX]
    ...
    def resolve_specific_epithet_suffixes(self, final_taxonomy):
        ...
        for rid in final_taxonomy:
            ...
            if canonical_taxon(gtdb_specific) != canonical_taxon(gtdb_specific_corr):
                ...
                if suffix is None:
                    final_taxonomy[rid][TaxonRank.SPECIES_INDEX] = gtdb_species_corr
                else:
                    final_taxonomy[rid][TaxonRank.SPECIES_INDEX] = '{}_{}'.format(
    ...
    def resolve_species_classification_ledger(self, final_taxonomy, cur_genomes, cur_clusters, species_classification_ledger, manual_curation_rids):
        ...
        for gid in final_taxonomy:
            target_domain = final_taxonomy[gid][TaxonRank.DOMAIN_INDEX]
            break
        ...
        for rid, taxa in final_taxonomy.items():
            final_species[rid] = taxa[TaxonRank.SPECIES_INDEX]
        ...
        for gid, mc_species in gtdb_sp_ledger.items():
            ...
            if final_sp in final_species and rid != final_species[final_sp]:
                self.log.error('Species {} proposed for {} in species classification ledger already exists in GTDB taxonomy: {}'.format(
                    final_sp, gid, final_species[final_sp]))

            if (rid in manual_curation_rids
                    and final_taxonomy[rid][TaxonRank.SPECIES_INDEX] != final_sp):
                self.log.warning('Deferring to manual curation over update suggested by species classification ledger {}: {} {}'.format(
                    rid, final_taxonomy[rid][TaxonRank.SPECIES_INDEX], final_sp))
            else:
                final_taxonomy[rid][TaxonRank.SPECIES_INDEX] = final_sp

            if is_latin_sp_epithet(gtdb_specific) and not test_same_epithet(gtdb_specific, mc_specific):
                if gid in final_taxonomy:
                    self.log.warning('Species classification ledger resulted in active change to Latin name of representative {}: {} -> {}'.format(
                        rid, gtdb_species, mc_species))
                else:
                    self.log.warning('Species classification ledger entry for {} resulted in active change to Latin name of cluster now represented by {}: {} -> {}'.format(
            ...
            fout.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(
                ...
                mc_species == final_taxonomy[rid][TaxonRank.SPECIES_INDEX],
                gtdb_species,
    ...
    def resolve_duplicate_type_strain_species(self, mc_taxonomy, cur_genomes):
        ...
        for rid, taxa in mc_taxonomy.items():
            if cur_genomes[rid].is_gtdb_type_strain():
                sp = taxa[TaxonRank.SPECIES_INDEX]
        ...
        for sp, rids in sp_gids.items():
            if len(rids) > 1:
                ...
                common_genera = []
                for rid in rids:
                    genus = mc_taxonomy[rid][TaxonRank.GENUS_INDEX]
                ...
                if len(common_genera) == 1:
                    for rid in rids:
                        if rid not in common_genera:
                            ...
                            mc_taxonomy[rid][TaxonRank.SPECIES_INDEX] = new_sp
    ...
    def finalize_species_name(self,
        ...
        if final_sp in cur_gtdb_sp:
            ...
            self.log.warning('Finalized GTDB species name {} already exists: rid={} prev_rid={} prev_case={}'.format(
    ...
    def classify_ncbi_species(self,
        ...
        self.log.info(' - resolved {:,} ({:.2f}%) of species names'.format(
        ...
        self.log.info('   - assigned {:,} ({:.2f}%) by majority vote'.format(
            ...
            for line in f:
                ...
                if rid in final_taxonomy:  # other representative must be from other domain
                    ...
                    if len(tokens) > warning_idx and tokens[warning_idx]:
                        self.log.warning('Assuming synonym priority for {} ({}) has been fixed manually.'.format(
                            final_taxonomy[rid][TaxonRank.SPECIES_INDEX],
                            rid))
    ...
    def update_curation_tree(self, curation_tree, final_taxonomy):
        ...
        self.log.info('Reading curation tree.')
    ...
    def run(self,
        ...
        for gid in gtdb_type_strain_ledger:
            ...
            cur_gtdb_sp = mc_taxonomy[gid][TaxonRank.SPECIES_INDEX]
        ...
        _ncbi_species, unambiguous_ncbi_sp, ambiguous_ncbi_sp = self.classify_ncbi_species(
            ncbi_species_mngr,
            ncbi_synonyms,
            ncbi_misclassified_gids)
        ...
        for rid, taxa in final_taxonomy.items():
            sp = taxa[TaxonRank.SPECIES_INDEX]
            if sp in sp_names:
                self.log.error('Species name {} assigned to at least 2 GTDB representatives: {} {}'.format(
                    sp, sp_names[sp], rid))
            sp_names[sp] = rid

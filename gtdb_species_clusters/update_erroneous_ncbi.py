...
class UpdateErroneousNCBI(object):
    ...
    def identify_misclassified_genomes_ani(self, cur_genomes, cur_clusters):
        ...
        for ncbi_species, species_gids in ncbi_sp_gids.items():
            ...
            if len(gids_to_check) > 0:
                self.logger.info('Using ANI to establish erroneous assignments of {}:'.format(ncbi_species))
                gid_pairs = []
    ...
    def run(self, gtdb_clusters_file,
        ...
        # identify genomes with erroneous NCBI species assignments
        self.logger.info('Identifying genomes with erroneous NCBI species assignments.')

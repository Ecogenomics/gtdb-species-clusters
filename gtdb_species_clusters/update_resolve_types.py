...
class ResolveTypes(object):
            ...
            if os.path.exists(ltp_file):
                with open(ltp_file) as f:
                    ...
                    for line in f:
                        ...
                        if ';type sp.|' in taxonomy:
                            taxa = taxonomy.split(';type sp.|')[0].split(';')
                        elif ';|' in taxonomy:
                            taxa = taxonomy.split(';|')[0].split(';')
                        elif '|' in taxonomy:
                            taxa = taxonomy.split('|')[0].split(';')
                        elif taxonomy[-1] == ';':
                            taxa = taxonomy[0:-1].split(';')
                        else:
                            taxa = taxonomy.split(';')
    ...
    def resolve_gtdb_species(self, gid_anis, ncbi_sp, type_gids, cur_genomes):
        ...
        for gid in type_gids:
            if ncbi_sp_epithet == cur_genomes[gid].gtdb_taxa.specific_epithet:
                ...
                if gtdb_sp != 's__':
                    gtdb_sp_rep = cur_genomes.gtdb_sp_rep(gtdb_sp)
                    if cur_genomes[gtdb_sp_rep].is_effective_type_strain():
                        # genome has been assigned to another species
                        # defined by a type strain genome
                        untrustworthy_gids[gid] = f'Conflicting GTDB species assignment of {cur_genomes[gid].gtdb_taxa.species}'
    ...
    def run(self, 
        ...
        # consider manually selected genome assemblies to be correct
        ...
        fout_genomes.write('Genome ID\tResolved\tNCBI species\tGTDB genus\tGTDB species\tLTP species\tMean ANI\tStd ANI\tMean AF\tStd AF\tExclude from RefSeq\tNCBI taxonomy\tGTDB taxonomy\n')
        
        fout_unresolved = open(os.path.join(self.output_dir, 'unresolved_type_strain_genomes.tsv'), 'w')
        fout_unresolved.write('Genome ID\tNCBI species\tGTDB genus\tGTDB species\tLTP species\tMean ANI\tStd ANI\tMean AF\tStd AF\tExclude from RefSeq\tNCBI taxonomy\tGTDB taxonomy\n')
            ...
            if not all_similar:
                ...
                if resolved:
                    ...
                    for gid, reason in untrustworthy_gids.items():
                        ...
                        if len(ltp_species) == 1 and ncbi_sp.replace('s__', '') in ltp_species:
                            other_sp = set()
                            for test_gid in type_gids:
                                ltp_species = self.ltp_species(test_gid, ltp_metadata)
                                if ltp_species and ncbi_sp.replace('s__', '') not in ltp_species:
        ...
        if unresolved_sp_count > 0:
            ...
            self.logger.warning('These must be handled before proceeding with the next step of GTDB species updating.')

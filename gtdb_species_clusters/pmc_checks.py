...
class PMC_Checks(object):
    ...
    def manual_species(self, init_taxonomy, manually_curated_tree):
        ...
        init_num_gids = sum([1 for gid in init_taxonomy if not gid.startswith('D-')])
        self.logger.info(' - read taxonomy for {:,} genomes.'.format(init_num_gids))
        ...
        self.logger.info(' - identified {:,} manually-curated species names.'.format(num_mc))
    ...
    def replace_generic(self, manual_species_names, manual_taxonomy):
        ...
        self.logger.info(' - read manually-curated species for {:,} genomes.'.format(len(mc_species)))
        ...
        mc_num_gids = sum([1 for gid in mc_taxonomy if not gid.startswith('D-')])
        self.logger.info(' - read taxonomy for {:,} genomes.'.format(mc_num_gids))
        
        # replace generic names with genus names
        self.logger.info('Creating taxonomy file with updated species names.')
        fout = open(os.path.join(self.output_dir, 'taxonomy_updated_sp.tsv'), 'w')
        num_genomes = 0
        for gid, taxa in mc_taxonomy.items():
            if gid.startswith('D-'):
                continue
            ...
            if not generic:
                self.logger.error('Genome is missing genus assignment: {}'.format(gid))
                raise
            ...
            if len(sp_tokens) < 2:
                self.logger.error('Species name appear to be erroneous: {} {}'.format(gid, species))
                specific = '<unassigned>'
            else:
